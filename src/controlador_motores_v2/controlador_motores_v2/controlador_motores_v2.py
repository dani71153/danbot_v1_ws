...
class MotorController(Node):
    ...
    def __init__(self):
        ...
        try:
            self.arduino_ser = serial.Serial(self.arduino_port, self.baud_rate, timeout=10)
        ...
        try:
            self.esp32_ser = serial.Serial(self.esp32_port, self.baud_rate, timeout=10)
    ...
    def listener_callback(self, msg):
        ...
        v1 = self.saturate_speed(msg.data[0]) if len(msg.data) > 0 else None
        v2 = self.saturate_speed(msg.data[1]) if len(msg.data) > 1 else None
        v3 = self.saturate_speed(msg.data[2]) if len(msg.data) > 2 else None
        v4 = self.saturate_speed(msg.data[3]) if len(msg.data) > 3 else None

        # Enviar comandos al Arduino (motores v1 y v2)
        if v1 is not None and v2 is not None and self.arduino_ser is not None:
            try:
        ...
        if v3 is not None and v4 is not None and self.esp32_ser is not None:
            try:
