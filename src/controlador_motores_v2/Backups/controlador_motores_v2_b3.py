...
class MotorController(Node):
    ...
    def __init__(self):
        ...
        # Temporizador para leer los encoders periódicamente (cada 0.1 segundos)
        self.create_timer(1, self.read_encoders)
    ...
    def listener_callback(self, msg):
        # Verificar que los puertos seriales están abiertos
        if len(msg.data) < 4:
            self.get_logger().error('Received message does not contain enough data')
            return
    ...
    def read_encoders(self):
        """Leer los valores de los encoders de los controladores."""
        encoder_values = []
        ...
        if self.arduino_ser is not None:
            try:
                ...
                if arduino_response:
                    try:
                        arduino_values = [float(val.replace(',', '.')) for val in arduino_response.split(',')]
        ...
        if self.esp32_ser is not None:
            try:
                ...
                if esp32_response:
                    try:
                        esp32_values = [float(val.replace(',', '.')) for val in esp32_response.split(',')]
